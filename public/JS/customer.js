$(function () {
  ...
  $("select.form-select, select.form-control")
    .not("#state, #city, #bank_name, #branch")
    .select2({
      placeholder: "Select",
      allowClear: true,
      width: "100%",
...
const validateSubmitCustomer = (event) => {
  ...
  // Clear previous static validation styling
  ...
  // check_validate also colors background, but we want Bootstrap style.
  // We can skip check_validate if we implemented the above loop correctly.
  // However, check_validate might do other things. For now, let's rely on our manual check above.
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      Swal.fire({
        title: "Processing...",
        html:
          "<p>The customer saving process may take some time depending on your document upload sizes.</p>" +
      ...
      $.ajax({
        ...
        success: function (data, textStatus, xhr) {
          if (xhr.status === 200) {
            // Check for approval (custom response 200 with approval message)
            if (
              data.requiresApproval ||
              (data.message && data.message.toLowerCase().includes("approval"))
            ) {
              Swal.fire({
                icon: "info",
              ...
              save_doc(data.id || 0, function () {
                save_bank(data.id || 0, function () {
                  Swal.fire({
                    position: "center",
                    icon: "success",
        ...
        error: function (xhr) {
          if (xhr.status === 422) {
            ...
            $(".invalid-feedback").html(""); // Clear previous
            ...
            $.each(errors, function (key, value) {
              ...
              if (input.length > 0) {
                ...
                // Determine where to show error
                var feedback = input.next(".invalid-feedback");
                if (feedback.length === 0) {
                  // Try getting it from parent if select2 or other wrapper
                  feedback = input.parent().find(".invalid-feedback");
                }
                feedback.html(value[0]).show();
              } else {
                // For select2 usually the select is hidden
                // We might need to handle select2 container border override or placing error after it
                // For now, simple fallback
                Swal.fire("Validation Error", value[0], "warning");
              }
            });
            Swal.fire(
              "Error!",
