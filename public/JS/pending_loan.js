...
function load_table() {
  ...
  table = $("#loan_table").DataTable({
    ...
    columns: [
      { data: "Loan_No", name: "customer_loan.Loan_No", width: "8%" },
      { data: "route_name", name: "route.name" },
      { data: "center_no", name: "center.No" },
      {
        data: "group_name",
        name: "group_name",
        orderable: false,
      ...
      { data: "customer_name", name: "customer.First_Name", orderable: false },
      { data: "cus_number", name: "customer.cus_number" },
      { data: "loan_name", name: "loan_category.Name" },
      { data: "Amount", name: "customer_loan.Amount" },
      { data: "Date_Time", name: "customer_loan.Date_Time" },
      { data: "reason", name: "customer_loan.reason", visible: true },
      { data: "lending_officer", name: "u2.Full_Name" },
      { data: "user_name", name: "u1.Full_Name" },
      {
    ...
    order: [[8, "desc"]], // Sort by Date_Time
    drawCallback: function (settings) {
      var json = settings.json;
      if (json) {
        $("#tot_amount").text(parseFloat(json.total_amount || 0).toFixed(2));
        $("#loan_count").text(json.recordsFiltered);
      }
    },
  });
}
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      $.each(response.installment, function (index, installment) {
        let totalBalance =
          parseFloat(installment.Interest_Balance) +
          parseFloat(installment.capital_balance);
        ...
        var row =
          ...
          "</td>" +
          '<td class="text-end">' +
          installment.Panelty_date +
          "</td>" +
          '<td class="text-end">' +
          installment.Panalty_Amount +
          "</td>" +
          '<td class="text-end">' +
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        ...
        error: function (xhr) {
          ...
          Swal.fire(
            "Error!",
            "Failed to update installments: " + xhr.responseText,
            "error"
          );
        },
      });
...
function addInstallmentDates() {
  ...
  if (startDate.trim() === "" || isNaN(new Date(startDate))) {
    ...
    isNaN(parseInt(installmentCount)) ||
    parseInt(installmentCount) <= 0
  ) {
    Swal.fire(
      "Error!",
      "Please enter a valid installment count greater than zero !",
      "error"
    );
  } else {
    ...
    if (loan_type === "Daily") {
      ...
      if (selected_date.getDate() === 1) {
        if (
          on_a_selected_date_txt.trim() === "" ||
          isNaN(new Date(on_a_selected_date_txt))
        ) {
          Swal.fire(
            "Error!",
        ...
        Swal.fire(
          "Error!",
          "Selected date is not the first day of the month !",
          "error"
        );
      }
      ...
      if (selected_date.getDate() === lastDayOfMonth.getDate()) {
        ...
        Swal.fire(
          "Error!",
          "Selected date is not the last day of the month !",
          "error"
        );
      }
...
function load_approval_check(id) {
  $.ajax({
      ...
      if (xhr.status === 200) {
        ...
        data.item.forEach(function (document, index) {
          let levelDesignations = data.designation
            .filter(
              (designationObj) => designationObj.level_id == document.level_id
            ) // Match correct level
          ...
          if (login_designation === "Admin") {
            ...
            if (
              levelDesignationArray.includes(userDesignation) &&
              document.user_id === 0
            ) {
        ...
        <td>${document.date}</td>
        <td>
            <button class="btn btn-info btn-sm" onclick="toggleChecklist(${
              document.level_id
            },${id})">
                View Checklist (<span id="checklist_progress_${
                  document.level_id
                }">0/0</span>)
            </button>
        </td>
    ...
    <tr id="checklist_row_${document.level_id}" style="display: none;">
        <td colspan="9">
            <div id="checklist_container_${
              document.level_id
            }" class="p-3 bg-light"></div>
        </td>
    </tr>`;
...
function markChecklistItem(itemId, levelId, currentStatus, loan_id) {
  ...
  const action =
    newStatus === 1 ? "mark this item as completed" : "remove the mark";
        ...
        success: function (data) {
          if (data.success) {
            ...
            Swal.fire(
              ...
              `The checklist item has been ${
                newStatus === 1 ? "marked as completed" : "unmarked"
              }.`,
              "success"
            );
          } else {
