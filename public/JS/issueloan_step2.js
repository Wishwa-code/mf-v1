$(document).ready(function() {
    let x = ["#loan_amount", "#panelty_amount", "#interest", "#qty_2"];
    decimalFormat(x);
    $("#weekly").hide();
    $("#daily").hide();
    $("#load_div").hide();
    ...
    // Format current date as yyyy-mm-dd (assuming you want this format)
    var formattedDate = currentDate.getFullYear() + '-' +
        ('0' + (currentDate.getMonth() + 1)).slice(-2) + '-' +
        ('0' + currentDate.getDate()).slice(-2);

    // Set the formatted date as the value of the input field with id "startdate"
...
function changeCategory(id){
    $.ajax({
        ...
        success: function(data, textStatus, xhr) {
            ...
            view_doc(id);
            calculateInterest();
            generateWitnessTabs(loanCategory.Witness_Count);
            ...
            $("#interest_amount_txt").text("Total Interest Amount");
            $("#panelty_amount_txt").text("Penalty Rate Per Installment (%)");
            $("#interest_txt").text("Interest Rate Per Installment (%)");
            ...
            if(loanCategory.Collection_Type==="Daily"){
                $("#weekly").hide();
                $("#first_of_the_month").hide();
                $("#end_of_the_month").hide();
                $("#twice_a_month").hide();
                $("#on_a_selected_date").hide();
                $("#daily").show();
            }else if (loanCategory.Collection_Type==="Weekly"){
                $("#weekly").show();
                $("#first_of_the_month").hide();
                $("#end_of_the_month").hide();
                $("#twice_a_month").hide();
        ...
        success: function (data, textStatus, xhr) {
            ...
            if (xhr.status === 200) {
                ...
                data.required_documents.forEach(function(document) {
                    ...
                    row.append('<td><button class="btn btn-danger" onclick="remove_doc('+document.idRequired_Documents+')">Delete</button></td>');
        ...
        if(loan_type==="Daily"){
            ...
            if (on_a_selected_date_txt.trim() === "" || isNaN(new Date(on_a_selected_date_txt))) {
                ...
                installmentDates.forEach(function(date) {
                    ...
                    var row = '<tr><td>' + count + '</td><td>' + date + '</td><td class="text-end">'+installmentAmount+'</td><td  class="text-end">' + panelty_date + '</td><td class="text-end">0.00</td><td class="text-end">'+installmentAmount+'</td><td class="text-end">0.00</td><td class="text-end">0.00</td><td class="text-end">'+installmentAmount+'</td><td class="text-end">'+installmentAmount+'</td><td class="text-center"><span class="px-1" style="background-color: #ff0000;border-radius: 10px; color: #ff0000;">-</span></td></tr>';
...
// Get the last day of the selected month
                    ...
                    installmentDates.forEach(function(date) {
                        ...
                        var row = '<tr><td>' + count + '</td><td>' + date + '</td><td class="text-end">'+installmentAmount+'</td><td class="text-end">'+panelty_date+'</td><td class="text-end">0.00</td><td class="text-end">'+installmentAmount+'</td><td class="text-end">0.00</td><td class="text-end">0.00</td><td class="text-end">'+installmentAmount+'</td><td class="text-end">'+installmentAmount+'</td><td class="text-center"><span class="px-1" style="border-radius: 10px; color: #000000;">-</span></td></tr>';
...
function save_doc(id) {
    ...
    $('input[type="file"]').each(function(index, element) {
        // Get the file, document ID, and document name
        var file = element.files[0];
        var documentId = $(element).data('document-id');
        var documentName = $(element).closest('tr').find('td:first').text().trim();

        // Append the file, document ID, and document name to the FormData object
