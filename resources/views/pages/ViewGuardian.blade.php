                        ...
                        <table id="customerTable" class="display nowrap table table-striped table-bordered" style="width:100%">
                            ...
                            @foreach($guardian as $customer)
                                <tr>
                                    ...
                                    @endif
                                    <td><button class="btn btn-success" onclick="change_status({{$customer->idGuardian}})">Change Status</button></td>
                                    <td>
                                        <div class="d-flex justify-content-center">
                                            <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#view-modal" onclick="load_document({{$customer->idGuardian}});">
                                                <i class="bi bi-eye fs-4"></i>
                                            </button>
                                            <button type="button" class="btn btn-light edit-btn" data-bs-toggle="modal" data-bs-target="#standard-modal"
                                                    data-first-name="{{$customer->First_Name}}" data-last-name="{{$customer->Last_Name}}" data-title="{{$customer->Title}}"
                                                    data-email="{{$customer->Email}}" data-contact-no="{{$customer->Contact_No}}" data-nic="{{$customer->Nic}}"
                                                    data-gender="{{$customer->Gender}}" data-dob="{{$customer->Dob}}" data-address="{{$customer->Address}}" data-city="{{$customer->City}}"
                                                    data-State="{{$customer->State}}" data-landline="{{$customer->Landline}}"
                                                    data-note="{{ $customer->Note !== null ? $customer->Note : '-' }}"
                                                    data-customer-id="{{$customer->idGuardian}}" data-longitude="{{$customer->Longitude}}" data-latitude="{{$customer->Latitude}}">
                                                <i class="bi bi-pencil fs-4"></i>
                                            </button>

                                            <button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#standard-modal_2" onclick="set_cus({{$customer->idGuardian}})">
                                                <i class="bi bi-camera fs-4"></i>
                                            </button>

                                            <button type="button" class="btn btn-danger" onclick="delete_garantor({{$customer->idGuardian}})">
...
@section('script')
    ...
    <i class="bi bi-trash"></i> <!-- Replace bi-bucket with bi-trash for trash bucket icon -->
        ...
        function delete_doc(id){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "GET",
                        url: "/guardiandocument/delete/"+id,
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
                        },
                        success: function (data, textStatus, xhr) {
        ...
        function update_cus() {
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "Do you want to update this Guarantee?",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                    ...
                    $.ajax({
                        ...
                        success: function (data, textStatus, xhr) {
                            if (xhr.status === 200) {
                                ...
                                Swal.fire("Error!", "This Guarantor is already used in loan!", "error");
                            }
                        },
                    });
                }
