    ...
    <div class="row align-items-center mb-4 mt-3">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <div>
                    <h4 class="page-title mb-1 fw-bold fs-3 text-dark">Create New Lead</h4>
                    <ol class="breadcrumb m-0 small text-muted">
                        ...
                        <li class="breadcrumb-item active text-primary">Create Lead</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <form id="leadForm" enctype="multipart/form-data">
                                ...
                                <div class="p-3 bg-light rounded-3 border d-flex flex-wrap align-items-center justify-content-between gap-3">
                                    ...
                                    <div class="d-flex gap-2">
                                        <button type="button" class="btn btn-primary btn-modern btn-sm" onclick="getLeadLocation()">
                                            <i class="bi bi-geo-alt-fill me-1"></i> Get GPS
                                        </button>
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                <div class="upload-area">
                                    ...
                                    {{-- Explicit Location Fields for Guardian/Guarantor if needed --}}
                ...
                <div class="d-grid mb-5">
                    <button type="button" class="btn btn-primary btn-lg py-3 rounded-pill shadow fw-bold" onclick="saveLead()" id="saveLeadBtn">
                        <i class="bi bi-check-circle-fill me-2"></i> Submit Lead Information
                    </button>
                </div>
            </form>
        </div>
...
@section('script')
{{-- Select2 JS is already included in admin layout --}}
...
<script>
    ...
    function getLeadLocation() {
        ...
        Swal.fire({
            ...
            if (result.dismiss === Swal.DismissReason.cancel) {
                ...
                if (bestPosition) {
                    ...
                    // It's possible we have NOTHING yet if they cancelled instantly
        ...
        watchId = navigator.geolocation.watchPosition(
            ...
            (error) => {
                ...
                // We don't stop strictly on error, we wait for timeout or next success, 
                // unless it's permission denied which is fatal.
        ...
        setTimeout(() => {
            if (watchId) {
                // Time is up, take whatever we have
        ...
        function finishWithPosition(pos) {
            ...
            // Close the modal programmatically - this might trigger 'undefined' dismiss usually
            Swal.close();

            if (pos) {
                // Slight delay to allow Swal to close smoothly before we do showLeadPosition logic
    ...
    function showLeadPosition(position) {
        ...
        fetch(geocodeUrl)
            ...
            .finally(() => {
                $('textarea[name="address"]').attr('placeholder', 'Full residential/business address');
            });
    }

    function showLeadError(error) {
        $('#lead_lat_display').text('Error');
        $('#lead_lng_display').text('Error');
        Swal.fire('Location Error', 'Unable to retrieve location. Please check GPS settings.', 'error');
    }

    // Generic specific location getter for specific fields
    function getSpecificLocation(latFieldId, lngFieldId, statusDivId) {
        const statusDiv = document.getElementById(statusDivId);
        statusDiv.innerHTML = '<small class="text-info"><span class="spinner-grow spinner-grow-sm me-1"></span>Locating...</small>';
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    statusDiv.innerHTML = '<small class="text-success fw-bold"><i class="bi bi-check-circle-fill"></i> Location Captured</small>';
                },
                (error) => {
                    statusDiv.innerHTML = '<small class="text-danger fw-bold"><i class="bi bi-exclamation-triangle"></i> GPS Error</small>';
                }, {
                    enableHighAccuracy: true
                }
            );
        } else {
            statusDiv.innerHTML = '<small class="text-danger">Geo not supported</small>';
        }
    }
            ...
            success: function(res) {
                if (res.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Lead Created!',
                        text: 'New lead has been successfully added.',
                    ...
                    btn.prop('disabled', false).html('<i class="bi bi-check-circle-fill me-2"></i> Submit Lead Information');
                }
            },
            error: function(xhr) {
                ...
                if (xhr.status === 422) {
                    ...
                    $.each(errors, function(key, value) {
                        ...
                        if (field.length > 0) {
                            ...
                            // Handle input groups and regular inputs
    ...
    // --- Webcam & File Handling (Shared Logic) ---
    // Same webcam logic as Agreements page
    ...
    async function startWebcam() {
        ...
        try {
            ...
            errorDiv.innerHTML = '<i class="bi bi-camera-video-off display-4"></i><br>Camera Access Denied';
        }
    }
    ...
    function triggerFileSelect(id) {
        document.getElementById(id).removeAttribute('capture');
        document.getElementById(id).click();
    }

    function triggerCamera(id, mode) {
        document.getElementById(id).setAttribute('capture', mode);
        document.getElementById(id).click();
    }
