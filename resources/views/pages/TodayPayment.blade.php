...
<script>
    ...
    function togglePaymentSections(value) {
        togglePaymentSectionsGeneric(value, "");
    }

    function togglePaymentSections_2(value) {
        togglePaymentSectionsGeneric(value, "_2");
    }
...
<script>
    ...
    function buildPaymentTypeSelect(selectId, isCollector, isCashier, onChangeFnName) {
        ...
        if (onChangeFnName && typeof window[onChangeFnName] === "function") {
            sel.onchange = function() {
                window[onChangeFnName](this.value);
            };
    ...
    document.addEventListener("DOMContentLoaded", function() {
        buildPaymentTypeSelect("payment_type", {
            {
                (int) $collector
            }
        }, {
        ...
        buildPaymentTypeSelect("payment_type_2", {
            {
                (int) $collector
            }
        }, {
...
<script>
    ...
    function validatePaymentTypeBlock(suffix) {
        ...
        if (type === "Cheque") {
            ...
            if (!bank?.value) {
                Swal.fire("Cheque", "Please select Cheque Issue Bank.", "warning");
                return false;
            }
            if (!nameOn?.value) {
                Swal.fire("Cheque", "Please enter Name on the Cheque.", "warning");
                return false;
            }
            if (!no?.value) {
                Swal.fire("Cheque", "Please enter Cheque Number.", "warning");
                return false;
            }
            if (!date?.value) {
                Swal.fire("Cheque", "Please select Cheque Date.", "warning");
                return false;
            }
            if (!chqT?.value) {
                Swal.fire("Cheque", "Please select Cheque Type.", "warning");
                return false;
            }
        }
    ...
    function load_payment_reciept(id) {
        ...
        $.ajax({
            ...
            success: function(data, textStatus, xhr) {
                ...
                if (xhr.status === 200) {
                    if (data.point_check === "1") $('#loyalty_section').show();
                    else $('#loyalty_section').hide();
