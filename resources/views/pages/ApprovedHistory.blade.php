...
<script>
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `<span class="badge bg-success">${data || row.typeid}</span>`;
                    }
                },
                {
                    data: 'data_time',
                ...
                },
                {
                    ...
                    render: function(data) {
                        return `<span class="text-success">${data || 'N/A'}</span>`;
                    }
                },
                {
                    data: 'comment',
                    name: 'comment',
                    render: function(data) {
                        return `<div style="max-width: 200px;"><small class="text-muted">${data || 'No comment'}</small></div>`;
                    }
                },
                {
                    data: 'description',
    ...
    // View details function (reused from pending logic if needed, or define specific approved details view)
    // Since the action button calls viewDetails, we need it here.
    function viewDetails(id, typeId) {
        // ... (Logic same as pending or similar, assuming same view routes work for history)
        // Copying essential logic or assume it is available globally? 
        // It's not global. I need to include it.
        // Referencing strict implementation from PendingApproval.
        // Simplification: trigger modal similar to Pending
    }
</script>
@include('pages.partials.view_details_script')
{{-- Assuming I should create a partial or duplicate the script. 
         Since I can't easily create a partial right now without extra steps, I'll inline the simplified version of viewDetails 
         or better, just copy the one from PendingApproval but maybe simpler.
         Actually, the user asked to "use yajra" regarding the table. 
         The viewDetails implementation is crucial for "Action" column in my controller response.
    --}}
<script>
    function viewDetails(id, type, typeId) {
        ...
        // Reusing the modal from PendingApproval (need to ensure the HTML exists in this page too)
        // I added the modal HTML above.
