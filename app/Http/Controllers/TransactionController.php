    ...
    public function create_hm(Request $request)
    {
        ...
        $loanQuery = tableWithBranch('customer_loan', 'customer_loan')
            ->leftJoin('installments', function ($join) use ($from_date) {
                ...
                // Date condition inside join to still allow unmatched loans
                if (!empty($from_date)) {
                    $join->whereDate('installments.Installment_Date', '=', $from_date);
                }
            })
            ->join('customer', 'customer_loan.Customer_idCustomer', '=', 'customer.idCustomer')
            ->leftJoin('group_has_customer', 'customer.idCustomer', '=', 'group_has_customer.cus_id')
            ->leftJoin('customer_group', 'group_has_customer.group_id', '=', 'customer_group.idCustomer_Group')
            ->leftJoin('center', 'customer_group.center_id', '=', 'center.idCenter')
            ->leftJoin('user', 'customer_loan.User_idUser', '=', 'user.id')
            ->where('customer_loan.Status', '=', '0')
            ->select(
                'customer.idCustomer',
                DB::raw('IFNULL(center.No, "-") as center_no'),
                ...
                'customer_loan.Installment_Amount as Installment_Amount',
                'customer_loan.Vehicle_No as Vehicle_No',
                DB::raw('IFNULL(customer_group.Group_No, "-") as group_name'),
...
// ✅ Filter by center
    ...
    public function repaymentSheet10(Request $request){
        $center = tableWithBranch('center')->get();
        // 🔑 Sheet-specific key for this page
        $sheetKey = 'rs10'; // change the alias as you like
        ...
        if ($center->isEmpty()) {
            ...
            $center_details = null; // Or any default value you want to assign
            $grouped_loans = array(); // Or any default value you want to assign
            return view('pages.RepaymentSheet10', compact('center', 'grouped_loans','center_details'));
        } else {
            // If $center is not empty, set the default center value
            $center_details = $request->center_details ?? $center[0]->idCenter;
        }
        ...
        // 👉 Pass $sheetKey so the view/JS can save to repayment_order_<sheetKey>
        return view('pages.RepaymentSheet10', compact(
            'order_by','sheetKey','center','grouped_loans','center_details',
            'center_no','center_name','printedBy','printedAt','name_mode'
        ));
    }
    ...
    public function dailyreport(Request $request){
        ...
        $loanQuery = tableWithBranch('installments','installments')
            ...
            ->select(
                ...
                DB::raw('ROUND(SUM(CASE WHEN installments.Installment_Date = CURDATE() THEN installments.Total_Balance ELSE 0 END), 2) as Today_installment'),
                DB::raw('ROUND(SUM(CASE WHEN installments.Installment_Date < CURDATE() THEN installments.Total_Balance ELSE 0 END), 2) as arrease'),
                DB::raw('MAX(installments.Installment_Date) as loan_ending_date'),
                DB::raw('(SELECT IFNULL(SUM(Balance), 0) FROM customer_saving_accounts 
          WHERE customer_saving_accounts.Customer_Id = customer.idCustomer 
          AND customer_saving_accounts.Status = 1) as last_saving_balance'),
