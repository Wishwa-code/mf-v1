        ...
        try {
            ...
            if ($collection_type === 'Per Month') {
                ...
                $advanceDueDate = function (int $i) use ($firstDT) {
                    $d = clone $firstDT;
                    $d->modify("+{$i} weeks")->setTime(0, 0, 0);
                    return $d;
                };
            } elseif ($collection_type === 'Twice A Month') {
                $ppy = 24;
                $advanceDueDate = function (int $i) use ($firstDT) {
                    $d = clone $firstDT;
                    $d->modify('+' . ($i * 14) . ' days')->setTime(0, 0, 0);
                    return $d;
                };
            } else {
                $ppy = 365;
            ...
            if (!is_null($bank)) {
                ...
                $this->bankLogController->index(
                    $Interest_Receivable_Suspense_AC,
                    "Issue Loan",
                    $bank_log_comment,
                    "-",
                    "debit",
